...
var neverPurgeBase = []string{
	...
	"e2fsprogs",
	"mount",
	// remastering tools: tailor must never uninstall itself
	"penguins-tailor",
	"tailor",
	"penguins-wardrobe",
	"wardrobe",
...
func purgeExplicit(toRemove []string) {
	...
	_ = utils.ExecTee("UCF_FORCE_CONFFOLD=1 DEBIAN_FRONTEND=readline apt-get autoremove -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' --purge -y", tailorLogFile)
}
