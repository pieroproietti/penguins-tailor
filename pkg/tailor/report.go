...
// wear run to a timestamped text file under /var/log/tailor/, and returns
...
func writeWearReport(r wearReport) (string, error) {
	...
	w := bufio.NewWriter(f)
	fmt.Fprintf(w, "tailor wear report -- %s\n", time.Now().Format("2006-01-02 15:04:05"))
...
func printDisplayManagerNotice() {
	...
	utils.LogNormal("%s%s%s", utils.ColorYellow, msg, utils.ColorReset)
}
